...
let rec exp_and_seq (d: Data) exp: ReturnCases =
    ...
    | Vars vars ->
        match vars_to_tyvars [] vars with
        | Succ args -> 
            let ty = List.map get_type args |> VarsT
            let empty_names = List.map (fun _ -> V "") args
            match bind_typed_vars d.env (empty_names, args) with
            | Succ(args, acc) -> RTypedExpr <| TyVars(args,ty)
            | Fail er -> RError er
        | Fail er -> RError er
    | ET exprs ->
