...
and exp_and_seq (d: Data) exp: ReturnCases =
    ...
    let rec match_tyvt_template traverse bind (eval_env: Env) (acc: Env) (l,r) = 
        let match_vv = match_tyvt_template traverse bind
        match get_type r with
        | VTT x -> 
            let r = List.mapi (fun i t -> TyIndexVT(r,TyLitInt i,t)) x
            traverse (match_v_template match_vv bind eval_env) acc (l,r)
    ...
    let rec match_vv_template traverse bind_vv match_vars match_et bind_tyvars (eval_env: Env) (acc: Env) (l,r) = 
        ...
        | VT _ as r -> match_vars eval_env acc (l,r)
        | ET _ as r -> match_et eval_env acc (l,r)
        | r -> bind_tyvars eval_env acc (l,r)

    let match_vv name_checker = 
        match_vv_template fold_2_er (bind_any name_checker) (match_vt name_checker) 
            (match_et name_checker) (bind_tyvt name_checker)
