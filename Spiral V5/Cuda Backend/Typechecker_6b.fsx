...
and exp_and_seq (d: Data) exp: ReturnCases =
    ...
    let bind_typedexpr (name_checker: HashSet<string>) acc (left_arg, ty_exp) =
        match left_arg with
        | V arg_name -> bind_typedexpr'' name_checker acc (arg_name,ty_exp)
        | x -> Fail <| sprintf "Expected V arg_name is pattern matching. Got: %A" x
    let bind_template bind_expr bind_typedexpr eval_env acc (left_arg, right_arg) =
        match left_arg with
        | V arg_name ->
    ...
    let match_v_template match_vv bind (acc: Env) = function
        ...
        | x, right_arg -> bind acc (x, right_arg)
    ...
    let bind_mset_template eval_env acc (arg_name: Expr, r) =
        let d = {d with env=eval_env; args=[]}
        match tev d arg_name with
        | RTypedExpr (TyIndexArray(_,_,lt) as v) | RTypedExpr (TyV(_,_,lt) as v) when is_const' lt = false && lt = get_type_postfix (get_type r) ->
