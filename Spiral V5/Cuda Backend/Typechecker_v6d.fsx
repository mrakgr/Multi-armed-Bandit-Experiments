...
and TypedExpr =
    ...
    | Inlineable' of Expr * Expr * bound_args: HashSet<TyV> * Env * Ty
    | Method' of name: string * args: Expr * body: Expr * bound_args: HashSet<TyV> * Env * Ty
...
let rec get_type = function
    | Inlineable'(_,_,_,_,t) | Method'(_,_,_,_,_,t) -> t
...
and exp_and_seq (d: Data) exp: TypedExpr =
    ...
    let make_tyv ty_exp = 
        ...
        d.bound_variables.Add v |> ignore
        v
    ...
    let process_tyvv r = 
        List.map (fun e -> 
            let e = make_tyv_and_push e
            TyV e) r

    let bind_typedexpr_inl name_checker match_vv acc arg_name ty_exp =
        dup_name_check name_checker arg_name <| fun _ ->
            match ty_exp with
            | TyVV (r,t) ->
                let r = process_tyvv r
                let l = List.map (fun _ -> B) r |> VV
                match_vv (Map.add arg_name (TyVV(r,t)) acc) l (TyVV(r,t))
            | TyV v as v' -> Map.add arg_name v' acc
            | Inlineable' _ | Method' _ -> // This case is just so the typed AST is not so noisy when printed.
                let v = make_tyv ty_exp
                Map.add arg_name (TyV v) acc
            | _ ->
                let v = make_tyv_and_push ty_exp
                Map.add arg_name (TyV v) acc
    
    let bind_typedexpr_method name_checker match_vv acc arg_name ty_exp =
        dup_name_check name_checker arg_name <| fun _ -> 
            match ty_exp with
            | TyVV (r,t) ->
                let r = process_tyvv r
    ...
    let rec match_vv_inl' dup_name_checker = match_vv traverse_inl (bind_typedexpr_inl dup_name_checker)
    let rec match_vv_method' dup_name_checker = match_vv traverse_method (bind_typedexpr_method dup_name_checker)
    ...
    let apply_inlineable (la,body,bound_variables,env,_) ra =
        tev {d with env = match_vv_inl env la ra; bound_variables = bound_variables} body

    let apply_method (name,la,body,bound_variables,initial_env,t as orig) ra =
        ...
        | false, _ ->
            ...
            let d = {d with env = if name <> "" then Map.add name (Method' orig) env else env
                            bound_variables = HashSet(bound_variables)
                            current_stack=s}
            let body = with_empty_seq d body
            let sole_arguments = filter_simple_vars bound_args
                        
            s.Clear()
            d.memoized_methods.[method_key] <- MethodDone(sole_arguments, body, Set(bound_variables))
    ...
    let apply_second apply_inl apply_method la ra =
        match la with
        | Inlineable'(a,b,c,d,e) -> apply_inl (a,b,c,d,e) ra
        | Method'(a,b,c,d,e,f) -> apply_method (a,b,c,d,e,f) ra
        | _ -> failwith "impossible"
...
let data_empty() = 
    {memoized_methods=d0();tagged_vars=d0();bound_vars=HashSet(HashIdentity.Structural)
