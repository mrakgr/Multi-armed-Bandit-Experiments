...
let cuda_expr<'t> expr = CudaExpr expr : CudaExpr<'t>

type CudaScalar<'t> = CudaScalar of name: string with
    member t.Name = t |> fun (CudaScalar name) -> name
let cuda_scalar<'t> name = CudaScalar name : CudaScalar<'t>
...
type CudaAr1D<'t> = CudaAr1D of CudaScalar<int> * name: string with
    ...
    member t.Access(x: CudaScalar<int>) =
        ...
        cuda_expr<'t> expr
let cuda_ar1d<'t> name = CudaAr1D name : CudaAr1D<'t>
...
let cuda_ar2d<'t> name = CudaAr2D name : CudaAr2D<'t>
