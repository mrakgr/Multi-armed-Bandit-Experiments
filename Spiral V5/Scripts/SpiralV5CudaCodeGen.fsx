...
module CudaCompiler =
    ...
    let map_redocol_map_module map_load_op reduce_op map_store_op block_size num_in num_out name =
        let in_group = CudaGroup(num_in,[CudaArray("x",CudaFloat,["num_cols";"num_rows"])])
        ...
        cuda_codegen [
            ...
            externCBlock [
                method_ CudaGlobal CudaVoid name [in_group; out_group] [
                    ...
                    for_ ([CudaVar("col",CudaInt),Value "blockIdx.x"]) (Var "col" .< Var "num_cols") [MAdd(Var "col",Value "gridDim.x")] [
                        ...
                        ifVoid 
                            ...
                            (map_store_op (group_to_explist in_group [Var "col"; Var "row"]) (Var "result"))
                            []
                        ]]]]]
                    ]]
                ]
