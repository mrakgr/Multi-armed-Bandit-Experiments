...
//                        ret)
    ...
    and apply_functiont d (initial_env,(name,l) as fun_key) args on_fail ret =
    ...
    and apply_named_tuple d name ra on_fail ret =
        ...
        v d.env oname (fun () -> d.on_type_er <| sprintf "Cannot find an application overload for the tuple %s. %s is not in scope." name oname) <| fun la ->
            apply d la ra on_fail ret

    and apply d la ra on_fail (ret: TypedExpr -> _) =
...
//        | x, ForModuleT n -> failwithf "Expected a module in module application. Got: %A" x
        | FunctionT x,_ -> apply_functiont d x ra on_fail ret
