...
and exp_and_seq (d: CudaTypecheckerEnv) exp: TypedCudaExpr =
    ...
    let case_r_inl recurse acc (l,ls) (r,rs) (t,ts) =
        match recurse acc l r with
        | Succ x_acc -> recurse x_acc (R ls) (TyVV(rs,VVT ts))
        | Fail _ as er -> er

    let case_r_method recurse acc (l,ls) (r,rs) (t,ts) =
        match recurse acc l r with
        | Succ (x, x_acc) ->
            match recurse x_acc (R ls) (TyVV(rs,VVT ts)) with
    ...
    let rec match_single case_r case_bind (acc: Env) l r =
        ...
        | RS x, r -> recurse acc (R [S x]) r
        // For the tail end members, Moves up a level if a R is in last position.
        // Binds the rest of the tuple to l if it is E or S.
        // Non-last members have standard semantics
        | R ([R [_] as l]), TyVV([r],VVT [_]) 
        | R ([l]), r -> recurse acc l r
        | R ([R (l :: ls)]), TyVV(r :: rs,VVT (t :: ts))
        | R (l :: ls), TyVV(r :: rs,VVT (t :: ts)) -> case_r recurse acc (l,ls) (r,rs) (t,ts)
        | R (l :: ls), r -> Fail <| sprintf "Cannot destructure %A." r
        // The empty case cannot be generated by the above patterns and is only used to match on
        // empty tuples
        | R [], TyVV([],VVT []) -> case_bind acc "" r
        | R [], _ -> Fail <| sprintf "Expected an empty tuple."
    ...
    | If(cond,tr,fl) ->
        ...
        if is_bool cond = false then failwithf "Expected a bool in conditional.\nGot: %A" (get_type cond)
            ...
            if type_tr = type_fl then tev d (Apply(Method("",[R [],T (TyIf(cond,tr,fl,type_tr))]),VV []))
    ...
    | While(cond,body,e) ->
        let cond, body = tev d (Apply(Method("",[R [],cond]),VV [])), with_empty_seq d body
