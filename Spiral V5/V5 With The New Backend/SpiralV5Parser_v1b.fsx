...
let expr_block expr =
    ...
    between_rounds (sepBy (cases_all expr) semicolon)
    |>> function 
        | _ :: _ as l -> 
            List.reduceBack process_parser_expr l
            |> function ParserExpr a -> a | _ -> failwith "Last member of a block must be an expression or the block must be empty. Got a statement instead."
        | _ -> VV []
