...
and JoinPointState<'b> =
    | JoinPointInEvaluation
    | JoinPointDone of 'b
...
and JoinPointDict<'b> = Dictionary<JoinPointKey, JoinPointState<'b>>
...
let spiral_peval (Module(N(module_name,_,_,_)) as module_main) = 
    ...
    let join_point_dict_method: JoinPointDict<_> = d0()
    let join_point_dict_closure: JoinPointDict<_> = d0()
    let join_point_dict_type: JoinPointDict<_> = d0()
    let join_point_dict_cuda: JoinPointDict<_> = d0()
    ...
    let rec boxed_type_open = function // Will diverge on degenerate cases.
        | BoxedTypeT x -> boxed_type_dict.[x] |> boxed_type_open
        | x -> x
    ...
    and is_unit = function
        | LitT _ -> true
        | UnionT _ | BoxedTypeT _ | DotNetTypeT _ | ClosureT _ | PrimT _ -> false
        ...
        let join_point_type d x = 
            ...
            let ret_ty = 
                ...
                | false, _ ->
                    ...
                    ty 
                | true, JoinPointInEvaluation -> BoxedTypeT join_point_key.Symbol
    ...
    ()
//        let join_point_memoize (join_point_dict: JoinPointDict<_,_>) memo_type used_vars d expr =
//            let key_args = nodify_memo_key (expr, d.env) 
//
//            match join_point_dict.TryGetValue key_args with
//            | false, _ ->
//                join_point_dict.[key_args] <- MemoMethodInEvaluation
//                let typed_expr = tev_method d expr
//                join_point_dict.[key_args] <- MemoMethod (memo_type, used_vars, typed_expr)
//                typed_expr, key_args
