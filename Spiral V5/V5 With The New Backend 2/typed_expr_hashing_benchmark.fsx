...
let vv' n () = vv 10 n
let v' t () = v t
let l = v' IntT |> vv' |> vv' |> vv' |> vv' |> vv' |> vv' |> fun x -> x()
...
#time
for i = 1 to 10 do hash_typed_expr l |> ignore // 0.2s
for i = 1 to 10 do l.GetHashCode() |> ignore // 0.5s
for i = 1 to 10 do free_vars l |> ignore // 4.7s
