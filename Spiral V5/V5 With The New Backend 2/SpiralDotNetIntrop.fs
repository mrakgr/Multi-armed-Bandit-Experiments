...
let rec prim_dotnet_type_to_ty (x: System.Type) on_fail =
    ...
    elif x.IsArray then arrayt(DotNetHeap, prim_dotnet_type_to_ty (x.GetElementType()) on_fail)
    ...
    elif x.IsByRef then arrayt(DotNetReference, prim_dotnet_type_to_ty (x.GetElementType()) on_fail) // Incorrect, but useful
    elif FSharp.Reflection.FSharpType.IsFunction x then 
        let a,b = FSharp.Reflection.FSharpType.GetFunctionElements x
        closuret (prim_dotnet_type_to_ty a on_fail) (prim_dotnet_type_to_ty b on_fail)
    else on_fail x
...
let rec ss_eval prim_dotnet_type_to_ty (d: SSEnvTerm) (x: SSExpr): Ty =
    let inline ss_eval d x = ss_eval prim_dotnet_type_to_ty d x
    let rec ss_compile_type_definition (d: SSEnvTerm) (x: Type): Ty =
        prim_dotnet_type_to_ty x <| fun (x: Type) ->
...
and ss_apply a args = 
    ...
    | SSTyLam(env,bnds,body) ->
        ...
        ss_eval prim_dotnet_type_to_ty d body
